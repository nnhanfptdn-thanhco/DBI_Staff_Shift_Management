...
\begin{itemize}
    \item \textbf{Dữ liệu tĩnh:} Thông tin Nhân viên và Ca làm việc do bộ phận Quản lý (HR) nhập trực tiếp.
    \item \textbf{Dữ liệu động:} Thẻ chấm công (CheckIn/CheckOut) được đồng bộ từ thiết bị máy chấm công (hoặc phần mềm nội bộ) gửi về máy chủ.
\end{itemize}

\subsection{Xử lý dữ liệu với Python (Data Processing)}
Nhóm sử dụng hai thư viện \texttt{Pandas} và \texttt{Numpy} của Python để thực hiện:
\begin{itemize}
    \item \textbf{Tính toán giờ làm (TotalHours):} Xử lý các phép trừ đối tượng DateTime, đặc biệt là các ca làm qua đêm.
    \item \textbf{Phân tích trạng thái (Status):} Sử dụng Numpy để dán nhãn "Đi trễ" hay "Đúng giờ" dựa trên mốc thời gian quy định của từng ca làm.
\end{itemize}

\section{Kết luận (Conclusion)}
Hệ thống \textbf{Staff Shift Management} đã đáp ứng tốt các yêu cầu cơ bản về quản trị cơ sở dữ liệu. Thông qua dự án này, nhóm đã nắm vững kiến thức về thiết kế ERD, triển khai mô hình quan hệ trên SQL Server và đặc biệt là tích hợp Python để xử lý, làm sạch dữ liệu thực tế.
